-- DA load forecast for a single forecast date, latest execution before cutoff (RTO only)
-- Placeholders: {target_date}, {forecast_date}, {max_execution_hour}
WITH ranked AS (
    SELECT
        forecast_rank,
    ...
    WHERE
        forecast_execution_date::DATE = '{target_date}'
