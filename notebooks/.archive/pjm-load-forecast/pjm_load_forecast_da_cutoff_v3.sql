-- PJM Load Forecast — DA Cutoff Analysis (v3: RTO only, forward dates)
-- Finds the last forecast vintage before 9am EST (DA bidding cutoff),
-- compares it to 12h/24h/48h earlier vintages, and computes deltas.
-- Restricted to RTO region and forecast_date >= today.
...
-- For each forecast_date, find the last execution before 9am EST on that date
-- For future dates, cap at 9am today so we don't pick up post-cutoff vintages
cutoff_vintages AS (
  SELECT
    forecast_date,
...
-- 12h lookback: last execution at or before cutoff - 12h
