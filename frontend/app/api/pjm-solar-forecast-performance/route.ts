...
export async function GET(request: Request) {
  ...
  try {
    const [forecastRes, actualsRes, datesRes] = await Promise.all([
      query(FORECAST_SQL, [targetDate]),
      query(ACTUALS_SQL, [targetDate]),
      query(AVAILABLE_DATES_SQL, []),
    ]);
    ...
    console.error("[pjm-solar-forecast-performance] DB query failed:", error);
    return NextResponse.json(
      { error: "Failed to fetch solar forecast performance data" },
      { status: 500 }
    );
  }
}
