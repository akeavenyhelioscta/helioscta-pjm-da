...
interface PerformanceEntry {
  ...
  forecastFtm: number | null;
  forecastBtm: number | null;
  actual: number | null;
  error: number | null;
...
export default function SolarForecastPerformance() {
  /* --- state --- */
  ...
  useEffect(() => {
    ...
    fetch(`/api/pjm-solar-forecast-performance?${params}`, { signal: controller.signal })
      .then((res) => {
  ...
  const performanceData = useMemo(() => {
    ...
    const forecastMap = new Map<number, { ftm: number; btm: number }>();
    for (const f of data.forecasts) {
      if (f.forecast_rank === 1) {
        forecastMap.set(Number(f.hour_ending), {
          ftm: Number(f.solar_forecast),
          btm: Number(f.solar_forecast_btm),
        });
      }
    }
    ...
    for (const a of data.actuals) {
      actualMap.set(Number(a.hour_ending), Number(a.solar));
    }

    const entries: PerformanceEntry[] = HOURS.map((h) => {
      const fc = forecastMap.get(h);
      const forecastFtm = fc ? fc.ftm : null;
      const forecastBtm = fc ? fc.btm : null;
      const forecastTotal = forecastFtm != null && forecastBtm != null ? forecastFtm + forecastBtm : null;
      const act = actualMap.get(h) ?? null;
      const err = forecastTotal != null && act != null ? act - forecastTotal : null;
      const errPct = err != null && forecastTotal != null && forecastTotal !== 0 ? (err / forecastTotal) * 100 : null;
      return { hour: h, forecastTotal, forecastFtm, forecastBtm, actual: act, error: err, errorPct: errPct };
    });
    return entries;
  }, [data]);
  ...
  const vintageData = useMemo(() => {
    ...
    for (const f of data.forecasts) {
      ...
      byExec.get(exec)!.set(Number(f.hour_ending), Number(f.solar_forecast) + Number(f.solar_forecast_btm));
    }
    ...
    const chartData: VintageEntry[] = HOURS.map((h) => {
      ...
      entry["Fuel Mix Actual"] = actual ? Number(actual.solar) : null;
      return entry;
    });
  ...
  function yDomain(entries: PerformanceEntry[]): [number, number] {
    ...
    for (const e of entries) {
      for (const v of [e.forecastTotal, e.forecastFtm, e.forecastBtm, e.actual]) {
        if (v != null) {
          if (v < lo) lo = v;
          if (v > hi) hi = v;
        }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-gray-500">
          Grid-scale + BTM forecast vs Fuel Mix actuals for PJM solar generation
        ...
        <>
          ...
          <div className="rounded-xl border border-gray-800 bg-[#0f1117] p-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm font-semibold text-gray-200">PJM Solar (RTO)</p>
            ...
            <ResponsiveContainer width="100%" height={350}>
              <LineChart data={performanceData.map((e) => ({
                hour: e.hour,
                "Total Forecast": e.forecastTotal,
                "Grid-Scale Forecast": e.forecastFtm,
                "BTM Forecast": e.forecastBtm,
                ...
                <Line type="monotone" dataKey="Total Forecast" stroke="#f59e0b" strokeWidth={2} strokeDasharray="5 3" dot={false} isAnimationActive={false} />
                <Line type="monotone" dataKey="Grid-Scale Forecast" stroke="#fbbf24" strokeWidth={1.5} strokeDasharray="3 3" dot={false} isAnimationActive={false} />
                <Line type="monotone" dataKey="BTM Forecast" stroke="#d97706" strokeWidth={1.5} strokeDasharray="3 3" dot={false} isAnimationActive={false} />
          ...
          <div className="overflow-x-auto rounded-xl border border-gray-800">
            <table className="text-sm border-collapse" style={{ minWidth: "1400px" }}>
              ...
              <tbody>
                {(() => {
                  const rows = [
                    { label: "Total Forecast", getValue: (e: PerformanceEntry) => e.forecastTotal },
                    { label: "Grid-Scale Forecast", getValue: (e: PerformanceEntry) => e.forecastFtm },
                    { label: "BTM Forecast", getValue: (e: PerformanceEntry) => e.forecastBtm },
      ...
      {!loading && !error && data && activeView === "evolution" && (
        <>
          ...
          <div className="rounded-xl border border-gray-800 bg-[#0f1117] p-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm font-semibold text-gray-200">PJM Solar (RTO) — Forecast Evolution</p>
              <span className="inline-block h-2 w-2 rounded-full bg-yellow-400" />
            </div>
            ...
            <table className="text-sm border-collapse" style={{ minWidth: "1400px" }}>
              ...
              <tbody>
                {(() => {
                  ...
                  for (const f of data.forecasts) {
                    const v = Number(f.solar_forecast) + Number(f.solar_forecast_btm);
                  ...
                  return tableRows.map((row, mi) => {
                    const hourVals: (number | null)[] = HOURS.map((h) => {
                      ...
                      return match ? Number(match.solar_forecast) + Number(match.solar_forecast_btm) : null;
                    });
