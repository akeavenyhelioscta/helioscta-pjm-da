-- Meteologica DA price forecast DA cutoff vintages.
...
-- Parameterized by {hub} (default SYSTEM).
with params as (
    select
        case
...
resolved_cutoffs as (
    ...
    left join meteologica_cleaned.meteologica_pjm_da_price_forecast_hourly src
        on src.hub = (select hub from params)
...
ranked as (
    ...
    join meteologica_cleaned.meteologica_pjm_da_price_forecast_hourly d
        on d.hub = (select hub from params)
