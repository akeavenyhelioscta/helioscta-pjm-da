-- PJM wind forecast DA cutoff vintages (RTO).
...
-- where forecast_date >= today. No region filter — PJM wind is RTO-level only.
with params as (
    select
        case
...
resolved_cutoffs as (
    ...
    left join pjm_cleaned.pjm_wind_forecast_hourly src
...
ranked as (
    ...
    join pjm_cleaned.pjm_wind_forecast_hourly d
