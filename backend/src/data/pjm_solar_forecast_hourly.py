"""Pull PJM official solar forecast (latest execution per target date)."""
...
def pull(
    ...
    """Pull latest PJM solar forecast from today onward, one row per (date, hour_ending)."""
    sql_file = SQL_DIR / "pjm_solar_forecast_rto_latest.sql"
    ...
    logger.info("Pulling PJM solar forecast: RTO, today onward")

    df = pull_from_db(query=query)
    df["forecast_date"] = pd.to_datetime(df["forecast_date"]).dt.date
    df = validate_source_frame(
        df=df,
        source_name="pjm_solar_forecast_latest",
        required_columns=["forecast_date", "hour_ending", "forecast_execution_datetime", "solar_forecast"],
...
def pull_da_cutoff_vintages(
    ...
    sql_file = SQL_DIR / "pjm_solar_forecast_da_cutoff_vintages.sql"
    overrides: dict[str, str | int | bool | None] = {}
    if sql_overrides:
        overrides.update(sql_overrides)
    query = render_sql_template(sql_file, overrides)
    logger.info("Pulling PJM solar DA cutoff vintages")
