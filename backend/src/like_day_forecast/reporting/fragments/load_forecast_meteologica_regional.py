"""Meteologica regional load forecast report — WEST, MIDATL, SOUTH.

One chart per region with Euro ensemble band, controlled by a shared
global date-filter bar.  Each chart has an independent ramp toggle.
...
def build_fragments(
    ...
    """Pull Meteologica forecast + Euro ensemble for WEST / MIDATL / SOUTH."""
    logger.info("Building Meteologica regional load forecast report...")
    ...
    # 2. Divider
    fragments.append("Regional Load Forecasts — Meteologica")
    ...
    for region in REGIONS:
        ...
        if df_meteo is not None and len(df_meteo) > 0:
            ...
            chart = _build_chart(chart_id, df_meteo, df_euro,
                                 f"Meteologica {label} — Load",
                                 "Meteologica Load", "#EF553B")
            fragments.append((f"Meteologica {label} Forecast{vintage}", chart, None))
        else:
            fragments.append((f"Meteologica {label} Forecast",
                              _empty(f"No Meteologica forecast data for {label}."), None))
...
# -- Single-source chart with optional Euro band -----------------------------
