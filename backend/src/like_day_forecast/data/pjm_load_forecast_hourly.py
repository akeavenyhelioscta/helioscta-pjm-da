...
def pull(region: str = "RTO", lookback_days: int = 30) -> pd.DataFrame:
    """Pull PJM load forecast, one row per (date, hour_ending)."""
    sql_file = SQL_DIR / "pjm_load_forecast_rto_hourly.sql"
    with open(sql_file, "r") as f:
        query = f.read()

    query = query.format(region=region, lookback_days=lookback_days)
    logger.info(f"Pulling PJM load forecast: {region}, last {lookback_days} days")
...
def pull_strip(region: str = "RTO") -> pd.DataFrame:
    ...
    sql_file = SQL_DIR / "pjm_load_forecast_strip_rto.sql"
    with open(sql_file, "r") as f:
        query = f.read()
