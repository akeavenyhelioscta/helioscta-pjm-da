...
def build(df_gas: pd.DataFrame) -> pd.DataFrame:
    """Build gas price features from pivoted gas price data.

    Args:
        df_gas: Gas prices with columns [date, gas_m3_price, gas_hh_price, gas_transco_z6_price].

    Returns:
        DataFrame indexed by date with gas features.
    """
    df = df_gas.copy()
    ...
    # Raw prices (already available as of trade day)
    ...
    if "gas_transco_z6_price" in df.columns:
        result["gas_transco_z6_price"] = df["gas_transco_z6_price"]
    ...
    logger.info(f"Built {n_features} gas features")
    return result
