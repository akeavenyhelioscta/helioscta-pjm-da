...
def rmae(y_true: np.ndarray, y_pred: np.ndarray, y_naive: np.ndarray) -> float:
    ...
    Naive = same hour 7 days ago (Lago 2021 recommended).
    """
...
def mape(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """Mean Absolute Percentage Error. Target: 6-7% for PJM."""
    mask = y_true != 0
    if not np.any(mask):
        return np.nan
...
def sharpness(lower: np.ndarray, upper: np.ndarray) -> float:
    """Average prediction interval width (narrower is better, given correct coverage)."""
    return np.mean(upper - lower)


def crps(y_true: np.ndarray, y_pred_df: pd.DataFrame, quantiles: list[float]) -> float:
    """Continuous Ranked Probability Score — approximated via trapezoidal integration of pinball losses."""
    losses = []
    for q in quantiles:
        col = f"q_{q:.2f}"
        if col in y_pred_df.columns:
...
def evaluate_forecast(
    ...
    Args:
        y_true: Actual values (raw $/MWh, not asinh-transformed).
        y_pred_df: DataFrame with columns q_0.01, ..., q_0.99, point_forecast.
        quantiles: List of quantile levels.
        y_naive: Naive forecast (same hour 7 days ago) for rMAE computation.
    ...
    # Point metrics on median (q=0.50) or point forecast
    ...
    # Coverage and sharpness for standard intervals
    intervals = [
        ("80pct", 0.10, 0.90),
        ("90pct", 0.05, 0.95),
        ("98pct", 0.01, 0.99),
