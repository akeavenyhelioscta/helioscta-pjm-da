...
Shifts observed weather aggregates back by 1 day so they align with the reference
date. For backtest this uses D+1 observed weather as a "perfect forecast" proxy
(30+ years). For production (today), D+1 observed weather doesn't exist yet —
the corresponding feature groups gracefully drop out via NaN-group exclusion.
"""
import pandas as pd
import numpy as np
import logging
...
def build(
    ...
    daily = df.groupby("date").agg(
        ...
        tgt_feels_like_daily_avg=("feels_like_temp", "mean"),
    )
